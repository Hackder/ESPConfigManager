...
ESPConfigManager/lib/ESPConfigManager/src/index.html
<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
    ...
    <script type="module">
      ...
      // Memoized Config Pane to prevent re-renders when logs update
      const ConfigPane = memo(({ config, setConfig, handleChange }) => {
        return h(
          Fragment,
          null,
          h(
      ...
      function LogViewer({ logs, onClear }) {
        ...
        const handleScroll = () => {
          ...
          // If user scrolled up a bit, disable auto-scroll
          // Tolerance of 10px
        ...
        return h(
          ...
          h(
            ...
            logs.length === 0 &&
              h(
                "div",
                { style: "color: #666; text-align: center; padding: 2rem;" },
                "Waiting for logs...",
              ),
            logs.map((log, i) =>
              h(
      ...
      function App() {
        ...
        useEffect(() => {
          ...
          // Flush buffer periodically to avoid UI freezing on high throughput
        ...
        // Stable handleChange for memoized ConfigPane
        ...
        return h(
          ...
          h(
            ...
            h(ConfigPane, { config, setConfig, handleChange }),
          ),

          // Resizer Handle (Desktop only via CSS)
          h("div", {
